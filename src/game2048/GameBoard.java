...
public class GameBoard {
	...
	private boolean move(int row, int col, int horisontalDirection, int verticalDirection, Direction dir){
		...
		int newRow = row;
		while(move){
			newCol += horisontalDirection;
			newRow += verticalDirection;
			if(checkOutofBounds(dir, newRow, newCol))break;
			if(board[newRow][newCol] == null){
				board[newRow][newCol] = current;
				board[newRow - verticalDirection][newCol - horisontalDirection] = null;
	...
	private void moveTiles(Direction dir){
		...
		if(dir == Direction.LEFT){
			...
			for (int row = 0; row < ROWS; row++) {
				for (int col = 0; col < COLS; col++) {
					if(!canMove){
						canMove = move(row, col, horisontalDirection, verticalDirection, dir);
					}
					else move(row, col, horisontalDirection, verticalDirection, dir);
				}
			}
		}
